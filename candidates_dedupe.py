...
merged, then authoritatively classified against rubric.md by a stronger model.
...
# rubric.md is the same classification rubric the main dedupe uses; we send it as
...
def classify_development(client, candidate, state, ev):
    """Classify ONE synthesized development against rubric.md (candidate-adapted).

    Returns {"competencies", "relevance", "topic_tags"}. rubric.md (with the
...
def main():
    ...
    # Classify every development against rubric.md — single-article ones included.
    ...
    print(f"Classified {len(all_devs)} developments against rubric.md")
