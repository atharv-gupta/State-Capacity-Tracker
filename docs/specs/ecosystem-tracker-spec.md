...
- New `tracker/ecosystem/sources.py` (§4) replaces `sources.py`.
...
## 5. Pipeline stages — `tracker/ecosystem/pipeline.py`
...
0. **Verify feeds** (§4 Task 0) → write `tracker/ecosystem/sources.py` with a dated
   verification note.
1. **Build `tracker/ecosystem/pipeline.py`** — lift the §3 helpers, drop the
