...
the ecosystem tracker model (`tracker/ecosystem/pipeline.py`).
...
- **Raw layer** — `tracker/candidates/pipeline.py` writes one row per ARTICLE to
...
- **Clean layer** — `tracker/candidates/dedupe.py` (weekly, Mondays) clusters raw rows
  ...
  rubrics/rubric.md** using a stronger model (sonnet). This second pass is where
...
### `Candidate Developments` (RAW feed; `tracker/candidates/pipeline.py`-written)
...
### `Candidate Events` (CLEAN feed; `tracker/candidates/dedupe.py`-written)
...
- **`tracker/candidates/seed.py`** — reads `data/candidates_seed.json` (researched roster),
...
- **`tracker/candidates/pipeline.py`** — for every non-withdrawn candidate in
  ...
  competencies + relevance using rubrics/rubric.md definitions adapted to candidates
...
- **`tracker/candidates/dedupe.py`** — the clean layer (sibling of `dedupe.py`). Reads
  ...
  against `rubrics/rubric.md` (sent with a short candidate adaptation: a stated
...
- **`tracker/candidates/platforms.py`** — the static scrape. Per candidate: fetch the
...
after `tracker/candidates/dedupe.py` so the clean table is fresh. `load_candidate_devs`
