...
Loads the rubric once (adaptation prefix + rubrics/rubric.md), and holds the JSON
...
with open(os.path.join(RUBRICS, "congress-adaptation.md")) as f:
    ADAPTATION = f.read()
with open(os.path.join(RUBRICS, "rubric.md")) as f:
    RUBRIC = f.read()

# The adaptation re-points the four competencies at the federal government and
# adds the Congress-only carve-outs; rubrics/rubric.md supplies the definitions and the
